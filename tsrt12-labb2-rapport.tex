...
%______________________________________________________
%
\input{tsrt12-lab2-titelpage.tex}
...
Till expeimentet användes:
		\begin{itemize}
			...
			%-----------------------------ha med här eller i källor?------------------------------------------
			%\item Boken \textsl{Reglerteknik grundläggande teori} av Torkel Glad och Lennart Ljung
		\end{itemize}
...
C är godtycklig konstant. Eftersom utsignalen defineras som överföringsfunktionen faltat med insignalen kunde utsignalen efter laplacetransformering istället skrivas med multiplikation mellan överföringsfunktion och insignal.
\begin{equation}
\delta_{h1}(s) = \frac{C * K_{övre}}{sT+1}\frac{1}{s}
\end{equation}
\\
 Där laplacetransformen för enhetssteget är 
 \begin{equation}
 \frac{C}{s}
 \end{equation}
 %--------ändra
och där {deltah1} står för avvikelsen från 10 cm på den övre tanken. Med en invers laplacetransformering tillbaka i tidsdomänen gavs ett uttryck för
  %----------------
stegsvaret.
 \begin{equation}
\delta_{u}(t) = C{\cdot}{K_{övre}}(1-e^{-\frac{t}{T}}) 
\end{equation}
Eftersom \texttt{e} enligt uttrycket avtog med --t/T och eftersom T söktes, sattes tiden t till T.
 \begin{equation}
\delta_{u}(t) = C{\cdot}{K_{övre}}(1-e^{-\frac{T}{T}}) =C{\cdot}{K_{övre}}(1-e^{-1}) = 0,63{K_{övre}}C 
\end{equation}
Eftersom stegsvaret går mot K\textsubscript{övre}*C då tiden går mot oändligheten, gavs enligt uttrycket ovan att T kunde utläsas vid 63\% av stegsvarets slutvärde. I \texttt{LeadLag} visades stegsvaret för h\textsubscript{ö}(t) och ur detta kunde T\textsubscript{r} avläsas. 
